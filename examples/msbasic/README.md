...
Each target is an `nt65.json` configuration, so `nt65 build --config cbmbasic2` builds one by
hand, and the editor's active configuration chooses the one it analyzes. From a build of this

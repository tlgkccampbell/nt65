...
/// instances, followed through address aliases and through the operands a macro call gave.
...
public static class AddressSymbols
{
    ...
    private static IEnumerable<Symbol> Collect(SemanticModel model, SyntaxNode node, Expansion? on, HashSet<Symbol> followed)
    {
        ...
        foreach (var name in names)
        {
            ...
            // A member of an instance is at a place in the instance's segment.
            if (symbol.Kind == SymbolKind.Member && name.ChildTokens.Length > 0
                && model.SymbolAt(name.ChildTokens[0]) is { Kind: SymbolKind.Instance } instance)
            {
                symbol = instance;
            }
            ...
            switch (symbol.Kind)
            {
                case SymbolKind.Label or SymbolKind.Proc or SymbolKind.Instance:
                    yield return symbol;
                    break;

...
/// <summary>
/// Represents a block, which holds its opener line, then its contents, then the closing
/// <c>}</c> line when it has one. A block closed by a continuation line (<c>} .else {</c>) has no
/// closer, because that line is the opener of the next block.
/// </summary>
internal sealed class GreenBlock : GreenNode
{
    ...
    /// <summary>Gets the opener line, the block's contents, and the closing line when it has one.</summary>
    ...
    /// <summary>Gets the kind of block, from the statement its opener line ends with.</summary>
    public BlockKind BlockKind => Opener.OpensBlockKind;

    /// <summary>Gets the line that opens the block, which determines what kind of block it is.</summary>

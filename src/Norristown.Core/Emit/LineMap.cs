...
/// the <c>line</c> records use for it; and <c>line</c> maps one line of the <c>.s</c>, counting
...
public static class LineMap
{
    ...
    public static OutputFile? For(OutputFile output)
    {
        ...
        text.Append(CultureInfo.InvariantCulture, $"file 0, \"{output.Source}\", {output.SourceSize}\n");
        for (var i = 0; i < output.LineSources.Count; i++)
        {
            if (output.LineSources[i] != 0)
                text.Append(CultureInfo.InvariantCulture, $"line {i + 1}, 0, {output.LineSources[i]}\n");
        }

...
public static class BuildCommand
{
    ...
    /// the exit code: 0 when it built, 1 when the program is wrong, 2 when the command is.
    /// </summary>
    public static int Run(string[] arguments, string directory, TextWriter output, TextWriter error)
    {
        switch (arguments)
        {
            case ["--help" or "-h"] or ["build", "--help" or "-h"]:
            ...
            case ["build", .. var rest]:
                if (CommandLine.Parse(rest, out var problem) is { } command)
                    return Build(command, Path.GetFullPath(directory), error);
    ...
    private static string Version() =>
        typeof(BuildCommand).Assembly.GetCustomAttribute<AssemblyInformationalVersionAttribute>()?.InformationalVersion
            .Split('+')[0] ?? "0.0.0";

    private static int Build(CommandLine command, string directory, TextWriter error)
    {

...
/// <summary>
/// Provides the call hierarchy, which lists the routines that call a routine and the routines it
/// calls. The call edges are the ones the flow analysis already found for the cost lenses, so the
/// hierarchy agrees with the cycle counts. A <c>jsr</c>, a tail jump, a <c>.next</c> under a call
/// and a <c>per</c>/branch pair each count as a call where the operand names a routine.
/// <para>
...
{
    /// <summary>
    /// Returns the routines a hierarchy may start from at <paramref name="position"/>. There is
    /// usually one or none, but on the name a <see cref="Family"/> binds, every routine in the
    /// family is returned.
    /// </summary>
        ...
        ProgramAnalysis analysis, SemanticModel model, int position)
    {
        ...
        // The name a family binds appears once but declares every instance, all on the
    ...
    /// <summary>
    /// Returns every routine in the program that calls the routine <paramref name="item"/> names,
    /// with the ranges of its calls.
    /// </summary>
    ...
    /// The token checked between files, since a program may hold hundreds of them.
    /// </param>
    ...
    /// <summary>
    /// Returns every routine called by the routine <paramref name="item"/> names, with the ranges
    /// of the calls.
    /// </summary>
    /// <param name="analysis">The program the routine is in.</param>
    /// <param name="item">The routine, as the server gave it to the client.</param>
    ...
    /// <summary>Checks whether a symbol is a routine, the kind of symbol a call hierarchy lists.</summary>
    private static bool IsRoutine(Symbol symbol) => symbol.Kind is SymbolKind.Proc or SymbolKind.ExternProc;

    /// <summary>
    /// Returns every call in the program as the calling routine, the routine called, and the
    /// range of the statement that makes the call. A call ends its basic block, so the block's
    /// last statement is the call.
    /// </summary>
    ...
    /// <summary>
    /// Returns the routines found, ordered by file and then by where each is declared, so that a
    /// list never shuffles.
    /// </summary>
    ...
    /// <summary>
    /// Returns the routine an item refers to. A client sends an item back as it was given, so the
    /// routine is found again by where it is declared. Every instance of a family is declared at
    /// the same place, so the name is compared too, to tell the instances apart.
    /// </summary>
    ...
    /// <summary>
    /// Returns the span of the routine's whole declaration, including its body, or null when the
    /// outline has no item for it.
    /// </summary>

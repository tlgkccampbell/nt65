...
public sealed class MacroRequestsTests
{
    ...
    public async Task DefinitionFromInsideABodyReachesTheFile()
    {
        ...
        var parameter = await client.DefinitionAsync(Uri, new Position(3, 11), timeout);
        Assert.NotNull(parameter);
        Assert.Equal(2, parameter.Range.Start.Line);

        // A call goes to the macro it names, without expanding anything.
        var macro = await client.DefinitionAsync(Uri, new Position(19, 4), timeout);
        Assert.NotNull(macro);
        Assert.Equal(2, macro.Range.Start.Line);
    }
    ...
    public async Task ReferencesToAParameterStayInItsBody()
    {
        ...
        var uses = await client.ReferencesAsync(Uri, new Position(2, 29), includeDeclaration: false, timeout);
        Assert.Equal([3], uses.Select(use => use.Range.Start.Line));
    }
    ...
    public async Task RenamingAParameterRewritesTheBody()
    {
        ...
        var edit = await client.RenameAsync(Uri, new Position(2, 29), "amount", timeout);
        Assert.NotNull(edit);
        Assert.Equal([2, 3], edit.Changes[Uri].Select(change => change.Range.Start.Line).Order());
    }
    ...
    public async Task RenamingALabelUsedInABlockArgumentWorks()
    {
        ...
        var uses = await client.ReferencesAsync(Uri, new Position(18, 0), includeDeclaration: true, timeout);
        Assert.Equal([18, 21], uses.Select(use => use.Range.Start.Line).Order());

        var edit = await client.RenameAsync(Uri, new Position(21, 12), "@here", timeout);
        Assert.NotNull(edit);
        Assert.Equal([18, 21], edit.Changes[Uri].Select(change => change.Range.Start.Line).Order());
    }
    ...
    public async Task ABodysLabelIsNotTheCallers()
    {
        ...
        var inTheBody = await client.ReferencesAsync(Uri, new Position(11, 0), includeDeclaration: true, timeout);
        Assert.Equal([11, 14], inTheBody.Select(use => use.Range.Start.Line).Order());
        Assert.DoesNotContain(inTheBody, use => use.Range.Start.Line == 18);
    }
    ...
    public async Task HoverOnACallDescribesTheMacro()
    {
        ...
        var hover = await client.HoverAsync(Uri, new Position(19, 4), timeout);

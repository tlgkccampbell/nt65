...
public sealed class PlantedMistakesTests
{
    ...
    [
        ...
        ("src/reset.nt65", "    jsr init_ppu", "    jsr nmi\n    jsr init_ppu",
            "`nmi` is an interrupt handler and cannot be called: it returns with `rti`, which would not return to the caller"),
        ("src/reset.nt65", "    cli\n    jmp main::main", "    cli\n    rts",
            "`reset` is declared `noreturn`, but `rts` returns from it"),
        ("src/bank1.nt65", "    phb\n    phk\n    plb\n    rep #$20", "    phb\n    rep #$20",
            "`spawn_points` is in segment \"BANK1\", which is in bank $81, but B is $80 here"),
        ("src/bank1.nt65", "    lda f:speeds,x\n    rtl", "    lda f:speeds,x\n    rts",
            "is far, so it returns with `rtl`"),
        ("src/gfx.nt65", "    pea $4300\n    pld\n    stz d:$4310", "    stz d:$4310",
            "`d:$4310` is reached through the direct page at $0000, which covers only $0000 to $00ff"),
        ("src/gfx.nt65", "    plb\n    ldx #.sizeof(oam_shadow)", "    plb\n    sta INIDISP\n    ldx #.sizeof(oam_shadow)",
            "$2100 is reachable only from banks $00-$3f, $80-$bf, but B is $7e here"),
        ("src/math.nt65", "    pea 0                       ; prod_hi\n", "",
            "frame `f` is 10 bytes, but only 8 bytes are pushed here"),
    ];

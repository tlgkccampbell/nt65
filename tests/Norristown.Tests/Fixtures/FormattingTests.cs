...
public sealed partial class FormattingTests
{
    ...
    /// <summary>
    /// Two properties must hold for a file nobody has finished typing. The syntax tree reproduces
    /// its text exactly, every token of every line included, and formatting is idempotent, so
    /// formatting the formatted text changes nothing. Both are checked over every source in the
    ...
    /// <summary>
    /// Returns the same file with its whitespace changed in three ways. One version has no
    /// indentation, and another has tabs and spaces before and after every line. The last has the
    /// gap closed up where a run of data lines aligns its directives in a column. Line breaks are
    /// left alone, because the formatter keeps the ones a file already has.
    /// </summary>
    ...
    /// <summary>Returns the first line that differs, which shows what the formatter changed.</summary>
    ...
    /// <summary>Matches a line's leading whitespace, which the formatter is free to choose.</summary>
    [GeneratedRegex(@"^[ \t]+", RegexOptions.Multiline)]
    private static partial Regex Indent();

    /// <summary>Matches one line's own text, without the break that ends it.</summary>
    [GeneratedRegex(@"^[^\r\n]*", RegexOptions.Multiline)]
    private static partial Regex Line();

    /// <summary>
    /// Matches the gap between a declaration's <c>:</c> and its directive, where a run of lines
    /// aligns the directives in a column.
    /// </summary>
    [GeneratedRegex(@":[ \t]{2,}(?=\.)")]

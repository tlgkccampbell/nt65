    ...
    public void GeneratedFilesMatchTheTable()
    {
        ...
        foreach (var folder in SyntaxWriter.Folders)
        {
            ...
            problems.AddRange(Directory.Exists(full)
                ? Directory.GetFiles(full, "*.cs").Select(Repo.Named).Where(name => !files.ContainsKey(name))
                    .Select(name => $"{name} is generated by nothing")
                : [$"{folder} is missing"]);
        }

        Assert.True(
            problems.Count == 0,
            $"the generated syntax is stale; run `pwsh scripts/generate-syntax.ps1`:\n{string.Join("\n", problems)}");
    }

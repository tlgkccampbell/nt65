...
public sealed class SyntaxListTests
{
    ...
    public void TheItemsAndSeparatorsReadTogetherInSourceOrder()
    {
        ...
        Assert.Equal([SyntaxKind.IdentifierName, SyntaxKind.Comma, SyntaxKind.IdentifierName],
    ...
    public void AListWithoutSeparatorsReadsItsItems()
    {
        ...
        var parent = HandBuilt.Over("a b",
            new Green.ErrorLineSyntax(new GreenList([HandBuilt.Name(tokens[0]), HandBuilt.Name(tokens[1])])));
        var items = HandBuilt.List<IdentifierNameSyntax>(parent, 0);
        Assert.Equal(2, items.Count);
        Assert.Equal(["a", "b"], items.Select(item => item.GetText()));
        Assert.Equal(new TextSpan(2, 1), items[1].Span);
        ...
        Assert.Equal(new TextSpan(0, 3), parent.SlotRed(0)!.Span);
    ...
    public void ATokenListReadsTheTokensOfAList()
    {
        ...
        var parent = HandBuilt.Over("nop lda", new Green.ErrorLineSyntax(new GreenList([tokens[0], tokens[1]])));
    ...
    public void AListNodeWithNothingInItReadsAsAnEmptyList()
    {
        ...
        var parent = HandBuilt.Over("()",
            new Green.ArgumentListSyntax(tokens[0], new GreenSeparatedList([]), tokens[1]));
    ...
    private static SeparatedSyntaxList<IdentifierNameSyntax> Arguments(string text)
    {
        ...
        var node = HandBuilt.Over(text,
            new Green.ArgumentListSyntax(tokens[0], builder.ToSeparatedList(), tokens[^1]));
        return HandBuilt.SeparatedList<IdentifierNameSyntax>(node, 1);
    }
}
